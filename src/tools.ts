...
export const SearchEmailsSchema = z.object({
  query: z.string().max(2000).describe("Gmail search query (e.g., 'from:example@gmail.com')"),
  maxResults: z.number().min(1).max(500).optional().describe("Maximum number of results to return (max 500)"),
});
...
export const BatchModifyEmailsSchema = z.object({
  messageIds: z.array(z.string()).max(1000).describe("List of message IDs to modify (max 1000)"),
...
export const BatchDeleteEmailsSchema = z.object({
  messageIds: z.array(z.string()).max(1000).describe("List of message IDs to delete (max 1000)"),
  batchSize: z.number().min(1).max(100).optional().default(50).describe("Number of messages to process in each batch (default: 50, max: 100)"),
});
...
export const ListInboxThreadsSchema = z.object({
  query: z.string().max(2000).optional().default('in:inbox').describe("Gmail search query (default: 'in:inbox')"),
  maxResults: z.number().min(1).max(500).optional().default(50).describe("Maximum number of threads to return (default: 50, max: 500)"),
});
